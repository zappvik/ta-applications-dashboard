...
export default async function ApplicationsPage() {
  ...
  const { data: allSelections } = await supabase.from('selections').select('application_id, subject, user_id')
  ...
  return (
    <div>
      ...
      <p className="mt-2 mb-6 text-gray-600 dark:text-gray-300">Here is a list of all {applications?.length || 0} applications submitted.</p>

      <InstructionsBanner />

      <div className="mt-8">
        <ApplicationsTable applications={applications || []} initialSelections={mySelections} />
      </div>
    </div>
  )
}
